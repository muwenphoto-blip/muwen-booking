...
import { loadFinanceSummary, type FinancePeriod } from '@/lib/admin/finance';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = await loadFinanceSummary(safePeriod, anchor || defaultAnchor);
    return NextResponse.json({ summary });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : '無法載入財務統計' },
      { status: 400 },
    );
  }
}
